...
<?php 
	...
	$Advertising = Category::$ADVERT;
    $Broadcasting = Category::$BROADCAST;
    $Jurnalistik = Category::$JURNAL;
    $Public_Relation = Category::$PR;
    $other = Category::$OTHER;

    //proses input dokumen yang akan diuji dengan upload file
	/*$dir_target = 'upload/proposal/';
	$rnd = rand(0,9999);
	$filename = $rnd.basename($_FILES['dokn']['name']);
	//$filename = basename($_FILES['dokn']['name']);
    $uploadfile = $dir_target.$filename;
    $proses = move_uploaded_file($_FILES['dokn']['tmp_name'], $uploadfile);*/

    //proses input dokumen yang akan diuji tanpa upload file
    $dir_target = 'upload/proposal/';
    $judul_proposal = $_POST['dokn'];
    $uploadfile = $dir_target.$judul_proposal;
    //$proses = move_uploaded_file($_FILES['dokn']['tmp_name'], $uploadfile);
	...
	//proses data training atau data uji
	$category = $classifier -> classify($output);
	//$category = $classifier -> train($output, $Advertising);
//$hitung = $classifier -> classify($category);
      ...
      <div class="card mb-3">
        ...
        <div class="card-body">
          ...
          <div class="ps">
          	...
          	<form action="uji_proposal" method="post" enctype="multipart/form-data">
          		<div class="auto">
          			<label class="control-label col-xs-3" ><b>Nama File:</b></label>
		        		<div class="col-xs-9">
							<input type="text" name="proposal" value="<?php echo $judul_proposal ?>" class="form-control" style="width:300px;" readonly> 
						</div><br>
					<label class="control-label col-xs-3" ><b>Konsentrasi:</b></label>
		        		<div class="col-xs-9">
							<input type="text" name="konsentrasi" value="<?php echo $category; ?>" class="form-control" style="width:300px;" readonly> 
						</div><br>
					<label class="control-label col-xs-3" ><b>Isi Proposal:</b></label>
		        		<div class="col-xs-9">
		        			<div style="height:500px;width:1030px;font:16px/26px Georgia, Garamond, Serif;overflow:scroll;">
							<span><?php echo $output; ?></span> 
						</div>
				</div>
				<input class="btn btn-primary" style="display:block; margin:20px auto;" type="submit" value="UPDATE" name="proses" />
			</form>
        ...
        <div class="card-footer small text-muted">Text Mining -> Naive Bayes Classifier</div>
      </div>
    </div>
    <!-- /.container-fluid-->
    <!-- /.content-wrapper-->
